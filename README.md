...
Design a practical AI-based Plant Disease Detection System using the following constraints:

The input must be a plant leaf image.

The system must preprocess the image before classification.

Use a CNN-based deep learning approach.

The system should support multiple plant disease classes.

Include image augmentation.

Include training, validation and testing.

Evaluate the model using accuracy, precision, recall and F1-score.

Include a confusion matrix for classification analysis.

The system should provide the predicted disease class.

The solution should be feasible for a final-year engineering project.

Mention challenges such as poor lighting, blurred images and complex backgrounds.

Suggest a simple deployment approach.

Keep the solution simple, practical and easy to implement.

Present the answer using the format:

**Problem → Architecture → Dataset → Preprocessing → Model → Training → Evaluation → Deployment → Limitations → Recommendation**

## AI-GENERATED OUTPUT

Problem

The objective is to identify plant diseases from leaf images using computer vision and deep learning.
```
Architecture
...
A labelled plant leaf image dataset should be used.
...
Each image should have a corresponding disease label.
...
Converted into suitable numerical values.

Prepared for CNN input.

Data Augmentation

The training dataset can be augmented using:
...
This helps the model handle different image conditions.

## Model

A CNN can be used for image classification.
...
Output Layer
```
The output layer contains neurons corresponding to the number of disease classes.
...
The model can be trained using an optimizer such as Adam.
...
The trained model can be deployed through a simple web application.
...
The system may face difficulties with:
...
A CNN-based model is suitable for a final-year engineering project.

The project can initially support a limited number of plant disease classes and can later be extended to more diseases.

## OBSERVATION

The constraint-based prompt generated a highly structured response.

The AI followed the specified constraints and provided information about:
...
Therefore, constraint-based prompting provides better control and completeness.
